 ...
 */
async function attempt(dest, ref, opts = {}) {
  ...
  try {
    ...
    // A serverless container is frozen after the response, so a setTimeout retry
    // would never fire. Leave it marked 'retrying' — the cron sweep re-queues it.
    if (opts.noRetryTimer) {
      console.warn(`[pipeline] ${ref} → ${dest.name} attempt ${attempts} failed: ${err.message}`);
      return;
    }
...
/**
 * Same delivery, but awaited — for serverless, where the container is frozen as
 * soon as the response is sent and any unawaited work simply never happens.
 * Only the first attempt is awaited; the scheduled sweep picks up the rest.
 */
export async function deliverAndWait(booking) {
  await Promise.allSettled(
    DESTINATIONS.map((dest) =>
      attempt(dest, booking.ref, { noRetryTimer: true }).catch((err) =>
        console.error(`[pipeline] unexpected error for ${booking.ref}:`, err)
      )
    )
  );
...
export async function retry(ref, { wait = false } = {}) {
  const booking = store.find(ref);
  ...
  for (const dest of DESTINATIONS) {
    ...
    const run = attempt(dest, ref, { noRetryTimer: wait });
    if (wait) await run.catch(() => {}); else run.catch(() => {});
  }
  return true;
}
